...
###
### Todo: Only difference with this and other files is "../template2.htm index.htm"
### and not "../template2.htm index.html", so make a checkbox which does that instead
### of separate files.
###
...
cp --update=none ../template2.htm index.htm
...
if [ "$type" == "software" ] || [ "$type" == "text" ] || [ "$type" == "data" ]; then sed -i "s/img src=\"IMAGE1\" alt=\"\" \//b>Cannot preview file.<\/b/g" index.htm; fi

# If type is video then write VIDEO element
if [ "$type" == "video" ]; then sed -i "s/img src=\"IMAGE1\" alt=\"\" \//video controls autoplay><source src=\"IMAGE1\" type=\"video\/mp4\"><\/video/g" index.htm; fi

# Write title to HTML
sed -i "s/META1/$titlesafe/g" index.htm

# Write image file name to HTML's IMG tag
sed -i "s/IMAGE1/.\/$filename/g" index.htm

# Write page creation timestamp to HTML
time="$(TZ=UTC date -u +%Y-%m-%d\ %H:%M:%S)"; sed -i "s/TIME1/$time UTC by <a href=\"https:\/\/arweave.net\/Te63EWzCvnn8__nDx1SIuPCWqlAL3_drW-TWQaPpQYk\">html-html-gen<\/a>/g" index.htm

# Write tags, if any, to HTML
if [ ! -z "$tags" ]; then sed -i "s/<\x21--br><div>Tags/<br><div>Tags/g" index.htm; sed -i "s/TAGS1<\/div-->/$tagssafe<\/div>/g" index.htm; fi

# Write description, if any, to HTML
if [ ! -z "$desc" ]; then sed -i "s/<\x21--br><div>Description/<br><div>Description/g" index.htm; sed -i "s/DESC1<\/div-->/$descsafe<\/div>/g" index.htm; fi

# Write media type to HTML
sed -i "s/<\x21--br><div>Type/<br><div>Type/g" index.htm
sed -i "s/TYPE1<\/div-->/$type<\/div>/g" index.htm

# Write folder name to HTML, and write index of index link to HTML
if [ ! -z "$dirnobase" ]; then sed -i "s/<\x21--br><div>In folder/<br><div>In folder/g" index.htm; sed -i "s/DIR1<\/div-->/$dirnobase<\/div><div>Return | <a href=\"..\">.. (go up one folder)<\/a><\/div>/g" index.htm; fi
...
# fails if whitespace is in any filename
